# ROS 2 Basics for Physical AI (Beginner Level)
...
# Beginner Explanation: ROS 2 Node for Basic Physical AI System
import rclpy
...
# Beginner Explanation: Create a QoS profile for sensor data (reliable, keep last 10 samples)
...
# Beginner Explanation: Create a QoS profile for command data (best effort, keep last 1 sample)
...
# Beginner Explanation: Create subscribers for sensor data
...
# Beginner Explanation: Create publishers for commands and status
...
# Beginner Explanation: Create a service server for basic commands
...
# Beginner Explanation: Create a parameter to control behavior
...
# Beginner Explanation: Create a timer for periodic tasks
        self.timer = self.create_timer(0.1, self.periodic_task)  # 10 Hz

    def joint_callback(self, msg):
        """Process joint state messages"""
        # Update internal joint position tracking
# Beginner Explanation: Update internal joint position tracking
        for i, name in enumerate(msg.name):
            if i < len(msg.position):
                self.current_joint_positions[name] = msg.position[i]

        # Log when we receive joint data
# Beginner Explanation: Log when we receive joint data
...
# Beginner Explanation: Check for balance issues
...
# Beginner Explanation: Calculate pitch and roll (simplified)
            pitch = 2.0 * (w * y - z * x)
            roll = 2.0 * (w * x + y * z)

            # Define safety thresholds
# Beginner Explanation: Define safety thresholds
...
# Beginner Explanation: In a real system, we might trigger safety procedures here
...
# Beginner Explanation: Get max velocity from parameters
        max_vel = self.get_parameter('max_velocity').value

        # Simple movement pattern based on mode
# Beginner Explanation: Simple movement pattern based on mode
        if self.robot_mode == 'active':
...
def main(args=None):
    ...
    # Create the node
# Beginner Explanation: Create the node
    robot_node = RobotBasicsNode()

    try:
        # Spin the node to process callbacks
# Beginner Explanation: Spin the node to process callbacks
        rclpy.spin(robot_node)
    except KeyboardInterrupt:
        robot_node.get_logger().info('Interrupted by user')
    finally:
        # Clean up
# Beginner Explanation: Clean up
